...
import { CONTENT_TYPE_LABELS, PROJECT_STATUS_LABELS, PROJECT_STATUS_COLORS } from '@csf-live/shared/constants'
...
export function ProjectDashboard({ slug }: ProjectDashboardProps) {
  ...
  function handleDescBlur() {
    ...
    if (project && description !== (project.description ?? '')) {
      updateProject.mutate({ projectId: project.id, patch: { description: description || undefined } })
    }
  }
  ...
  const projectPresence = presenceList.filter(
  ...
  const recentContentItems = recentItems
    .filter(i => i._sourceTable === 'content_item')
    .slice(0, 5)
      ...
      <div>
        {editingTitle ? (
          ...
          <h1
            ...
            className="cursor-text text-lg font-semibold text-warm-900 dark:text-warm-100 hover:bg-warm-100 dark:hover:bg-warm-800 rounded-md px-2 py-1 transition-colors"
          >
            {project.title}
          </h1>
        )}
      ...
      <div>
        {editingDesc ? (
          ...
          <p
            ...
            className={cn(
              'cursor-text text-sm rounded-md px-2 py-1 transition-colors hover:bg-warm-100 dark:hover:bg-warm-800',
              description
      ...
      {projectPresence.length > 0 && (
        <div className="flex items-center gap-2">
          {projectPresence.map(p => {
            ...
            return (
              <div key={p.userId} className="flex items-center gap-1.5 text-xs text-warm-600 dark:text-warm-400">
